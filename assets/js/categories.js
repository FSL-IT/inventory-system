        ...
        renderRow: (c) => `
            <tr>
                <td style="font-weight:600">
                    <i class="bi bi-tag" 
                       style="color:var(--purple);margin-right:6px;font-size:12px">
                    </i>
                    ${escapeHtml(c.name)}
                </td>
                <td>
                    ${c.asset_count > 0
                        ...
                        : `<span style="color:var(--white-4);font-size:12px">
                               —
                           </span>`
                    }
                </td>
                ...
                <td>
                    <div class="table-actions">
                        <button class="btn btn-secondary btn-sm" 
                            onclick="openEditCategory(${c.id}, '${escapeJsStr(c.name)}')">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn btn-danger btn-sm" 
                            ${c.asset_count > 0 ? 'disabled title="Reassign assets first"' : ''} 
                            onclick="deleteCategory(${c.id}, '${escapeJsStr(c.name)}')">
...
function deleteCategory(id, name) {
    showConfirm(
        'Delete Category', 
        `Delete "${name}"? This fails if assets still use it.`, 
        async () => {
            try {
                await apiFetch(
                    `/src/api/categories.php?id=${id}`, 
