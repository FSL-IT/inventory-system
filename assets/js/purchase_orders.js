...
var filteredPOs    = [];
var poCurrentPage  = 1;
var poItemsPerPage = 25;
var poSort         = 'po.created_at';
var poDir          = 'desc';
var poViewId       = null; 

var MAX_CAT_CHIPS     = 3;
var ENDORSE_WARN_DAYS = 3;

var debouncedApplyPoFilters = debounce(() => {
    poCurrentPage = 1;
...
async function fetchInitialPOs() {
    ...
    tbody.innerHTML = `
        <tr>
            <td colspan="8" class="cell-date" style="text-align:center;padding:30px">
...
function clearPoFilters() {
    ['po_search', 'filter_vendor', 'filter_endorsed', 'filter_category', 'filter_owner', 'filter_fiscal_year']
...
function updatePoSortIcons() {
    ...
    if (active) {
        active.className = `bi ${poDir === 'asc' ? 'bi-sort-up' : 'bi-sort-down'} sort-icon sort-active`;
    }
}
...
function applyClientPoFilters() {
    ...
    filteredPOs = allPOs.filter(p => {
        ...
        if (endorsed === 'yes') matchEndorsed = !!p.date_endorsed;
        else if (endorsed === 'no') matchEndorsed = !p.date_endorsed;
        else if (endorsed === 'overdue') matchEndorsed = !p.date_endorsed && days > ENDORSE_WARN_DAYS;

        const matchCat = !catFilter || (p.categories && p.categories.toLowerCase().includes(catFilter));
        const matchOwner = !ownFilter || (p.owners && p.owners.toLowerCase().includes(ownFilter));
        const matchFy = !fyFilter || (p.fiscal_year && p.fiscal_year.toUpperCase() === fyFilter);

        return matchSearch && matchVendor && matchEndorsed && matchCat && matchOwner && matchFy;
    });

    const SORT_MAP = {
        'po.po_number': 'po_number', 'v.name': 'vendor_name', 'asset_count': 'asset_count',
        'po.date_received': 'date_received', 'po.date_endorsed': 'date_endorsed', 'po.created_at': 'created_at'
    };
...
function renderCurrentPoPage() {
    ...
    const pageData = filteredPOs.slice(startIdx, startIdx + poItemsPerPage);

    renderPoTable(pageData);

    const mockPg = { page: poCurrentPage, per_page: poItemsPerPage, total: totalItems, total_pages: totalPages };
...
function renderPoCounter(pg) {
    ...
    if (!el || !pg) return;
    if (!pg.total) { el.textContent = 'No results'; return; }
...
function buildCatChips(categoriesStr) {
    if (!categoriesStr || categoriesStr === '—') return '<span class="cell-date">—</span>';
    const cats = categoriesStr.split(', ');
    const shown = cats.slice(0, MAX_CAT_CHIPS).map(c => `<span class="tag tag-category">${escapeHtml(c)}</span>`).join('');
    const more = cats.length > MAX_CAT_CHIPS ? `<span class="tag">+${cats.length - MAX_CAT_CHIPS}</span>` : '';
    return shown + more;
}
...
function buildAgeTag(po) {
    ...
    return `<span class="tag tag-defective" title="${days} days since received"><i class="bi bi-exclamation-circle"></i> ${days}d overdue</span>`;
}
...
function endorsePO(id) {
    ...
    if (!po) return;
    showConfirm('Endorse PO', `Mark PO "${po.po_number}" as endorsed today?`, async () => {
        try {
            const res = await apiFetch(`/src/api/purchase_orders.php?id=${id}&action=endorse`, { method: 'PUT' });
...
async function viewPO(id) {
    poViewId = id;
    const po = allPOs.find(x => x.id === id);
    if (!po) { showToast('Could not find PO.', 'error'); return; }

    renderViewPoSummary(po);
    openModal('view_po');

    const assetBody = document.getElementById('view_po_asset_body');
    if(assetBody) assetBody.innerHTML = `<tr><td colspan="6" class="cell-date" style="text-align:center;padding:16px"><i class="bi bi-arrow-repeat spin"></i> Loading items…</td></tr>`;

    try {
        const data = await apiFetch(`/src/api/purchase_orders.php?id=${id}&action=assets`);
        renderViewPoAssets(data.data || []);
    } catch (err) {
        console.error('viewPO assets error:', err);
        if(assetBody) assetBody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:16px;color:var(--red)">Failed to load items.</td></tr>`;
    }
}
...
function renderViewPoSummary(po) {
    ...
    const dateEndorsed = po.date_endorsed ? formatDate(po.date_endorsed) : '⏳ Pending';
    const fyRow = po.fiscal_year ? `<div class="form-field"><label>Fiscal Year</label><div class="info-field"><div class="val">${escapeHtml(po.fiscal_year)}</div></div></div>` : '';

    bodyEl.innerHTML = `
        <div class="field-grid">
            <div class="form-field"><label>PO Number</label><div class="info-field"><div class="val val-mono">${escapeHtml(po.po_number ?? '—')}</div></div></div>
            <div class="form-field"><label>Vendor</label><div class="info-field"><div class="val">${escapeHtml(po.vendor_name ?? '—')}</div></div></div>
        </div>
        <div class="field-grid">
            <div class="form-field"><label>Date Received</label><div class="info-field"><div class="val">${formatDate(po.date_received)}</div></div></div>
            <div class="form-field"><label>Date Endorsed by Admin</label><div class="info-field"><div class="val">${dateEndorsed}</div></div></div>
        </div>
        ${fyRow}`;
}
...
function renderViewPoAssets(rows) {
    ...
    if (!rows.length) {
        tbody.innerHTML = `<tr><td colspan="6" class="cell-date" style="text-align:center;padding:16px">No assets linked to this PO yet.</td></tr>`;
        return;
    }
 
    const titleEl  = document.getElementById('view_po_title');
    const poNumber = titleEl ? titleEl.textContent.replace('📋 PO: ', '').trim() : '';
    const searchUrl = `/src/views/assets.php?search=` + encodeURIComponent(poNumber);
 
    tbody.innerHTML = rows.map(r => `
        <tr class="clickable-row" onclick="window.location.href='${searchUrl}'" title="View assets for this PO in inventory">
            <td><span class="tag tag-category">${escapeHtml(r.category ?? '—')}</span></td>
            <td class="cell-sm">${escapeHtml(r.description ?? '—')}</td>
            <td style="text-align:center"><span class="tag tag-deployed">${escapeHtml(r.quantity ?? 0)}</span></td>
...
function openAddPO() {
    ...
    ['po_number', 'po_date_received', 'po_date_endorsed', 'po_vendor'].forEach(id => safeSetVal(id, ''));
    openModal('add_po');
}

function openEditPO(id) {
    const po = allPOs.find(x => x.id === id);
    if (!po) { showToast('Could not load PO for editing.', 'error'); return; }
...
async function savePO() {
    ...
    if (!payload.po_number) { showToast('PO number is required.', 'error'); return; }

    const isEdit = !!id;
    try {
        await apiFetch(isEdit ? `/src/api/purchase_orders.php?id=${id}` : '/src/api/purchase_orders.php', { method: isEdit ? 'PUT' : 'POST', body: JSON.stringify(payload) });
        closeModal('add_po');
...
function exportPoTracker() {
    window.location.href = '/src/api/import_export.php?action=export_po_tracker';
}
...
async function populatePoVendorDropdown() {
    try { const d = await apiFetch('/src/api/vendors.php?per_page=500'); appendOptions('filter_vendor', d.data ?? [], 'id', 'name'); } catch (err) {}
}
async function populatePoFormVendors() {
    try { const d = await apiFetch('/src/api/vendors.php?per_page=500'); appendOptions('po_vendor', d.data ?? [], 'id', 'name'); } catch (err) {}
}
async function populatePoCategoryFilter() {
    try { const d = await apiFetch('/src/api/categories.php?per_page=200'); appendOptions('filter_category', d.data ?? [], 'name', 'name'); } catch (err) {}
}
async function populatePoOwnerFilter() {
    try { const d = await apiFetch('/src/api/process_owners.php?per_page=500'); appendOptions('filter_owner', d.data ?? [], 'name', 'name'); } catch (err) {}
}
async function populatePoFiscalYearFilter() {
    const el = document.getElementById('filter_fiscal_year');
    if (!el) return;
    try {
        const d = await apiFetch('/src/api/purchase_orders.php?per_page=5000');
        const years = [...new Set((d.data ?? []).map(p => p.fiscal_year).filter(Boolean))].sort().reverse();
        years.forEach(fy => {
            const opt = document.createElement('option'); opt.value = fy; opt.textContent = fy; el.appendChild(opt);
        });
    } catch (err) {}
}
...
function openPoImportModal() {
    ...
    if (submitBtn) { submitBtn.disabled = true; submitBtn.onclick = submitPoImport; }
...
async function submitPoImport() {
    ...
    try {
        const res = await fetch('/src/api/import_export.php?action=import', { method: 'POST', headers: { 'X-CSRF-Token': getCsrfToken() }, body: fd });
        ...
        if ((json.data?.success ?? 0) > 0) fetchInitialPOs();
    } catch (err) {
        showImportStep('upload');
