...
window.initProcessOwners = function() {
            ...
            <tr>
                <td style="font-weight:600">
                    <i class="bi bi-people" 
                       style="color:var(--yellow);margin-right:6px;font-size:12px">
                    </i>
                    ${escapeHtml(o.name)}
                </td>
                <td>
                ...
                <td>
                    <div class="table-actions">
                        <button class="btn btn-secondary btn-sm" 
                            onclick="openEditOwner(${o.id}, '${escapeJsStr(o.name)}')">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn btn-danger btn-sm" 
                            onclick="deleteOwner(${o.id}, '${escapeJsStr(o.name)}')">
...
function openAddOwner() {
    ...
    document.getElementById('owner_modal_title').textContent = 
        '🏢 Add Process Owner';
...
function deleteOwner(id, name) {
    showConfirm(
        ...
        `Delete "${name}"? Linked assets will become unassigned.`, 
        async () => {
            try {
                await apiFetch(
                    `/src/api/process_owners.php?id=${id}`, 
